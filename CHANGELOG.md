...
- **Evaluation Framework**: `query_engine.testing.cli` evaluation system using "golden datasets" to securely score LLM generation accuracy, completeness, and confidence calibration.
...
- Strictly uncoupled architecture to ensure API keys are injected at runtime exclusively via environment variables (`QUERY_ENGINE_OPENAI_API_KEY`) or securely initiated configurations.
