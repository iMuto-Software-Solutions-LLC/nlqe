...
from query_engine.utils import FileNotFoundError, SchemaError, get_logger
...
class DataSourceIntrospector:
    ...
    def __init__(self, path: str, datasource_type: str | None = None) -> None:
    ...
    def _infer_type(self, path: str, hint: str | None) -> DataSourceType:
    ...
    def _get_connection(self) -> Any:
        ...
        if self._connection is None:
            if getattr(self, "datasource_type", None) == DataSourceType.DUCKDB:
    ...
    def introspect(self, name: str = "", description: str | None = None) -> DataSourceSchema:
        ...
        """
        logger.info(f"Introspecting {self.datasource_type.value} datasource: {self.path}")
        ...
        try:
            ...
            if self.datasource_type == DataSourceType.PARQUET:
                ...
                conn.execute(f"CREATE OR REPLACE TABLE {table_name} AS SELECT * FROM '{safe_path}'")
                tables = [table_name]
    ...
    def _introspect_table(self, conn: Any, table_name: str) -> TableInfo:
        ...
        """
        # Get row count
        row_count_result = conn.execute(f"SELECT COUNT(*) FROM {table_name}").fetchall()
